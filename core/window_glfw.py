...
class WindowGLFW:
    ...
    def __init__(self,
        ...
        glfw.set_key_callback(self.window_glfw, self.callback_key)
        glfw.set_cursor_pos_callback(self.window_glfw, self.callback_mouse_move)
        glfw.set_mouse_button_callback(self.window_glfw, self.callback_mouse_button)
        glfw.set_window_size_callback(self.window_glfw, self.callback_window_resize)
        glfw.set_scroll_callback(self.window_glfw, self.callback_mouse_scroll)
        glfw.set_framebuffer_size_callback(self.window_glfw, self.callback_framebuffer_size)
        glfw.set_drop_callback(self.window_glfw, self.callback_drop_files)
    ...
    def callback_key(self, glfw_window, key, scancode, action, mods):
        if action == glfw.PRESS:
            self.keyboard_state[key] = constants.KEY_STATE_DOWN
        if action == glfw.RELEASE:
            self.keyboard_state[key] = constants.KEY_STATE_UP

    def callback_mouse_button(self, glfw_window, button, action, mods):
    ...
    def callback_mouse_move(self, glfw_window, x, y):
        self.mouse_state[constants.MOUSE_POSITION] = (x, y)

    def callback_mouse_scroll(self, glfw_window, x_offset, y_offset):
        self.mouse_state[constants.MOUSE_SCROLL_POSITION] += y_offset

    def callback_window_resize(self, glfw_window, width, height):
        self.window_size = (width, height)

    def callback_framebuffer_size(self, glfw_window, width, height):
        raise NotImplementedError('[ERROR] Function not implemented')
    
    def callback_drop_files(self, glfw_window, file_list):
        raise NotImplementedError('[ERROR] Function not implemented')
