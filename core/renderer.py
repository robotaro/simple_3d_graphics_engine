...
class Renderer:
    ...
    def render_outline(self, nodes, color, camera: CameraInterface, viewport):
        ...
        self.window.fbo.depth_mask = True
    ...
    def render(self, scene: Scene, **kwargs):
        ...
        self.window.fbo.viewport = [0, 0, self.window.buffer_size[0], self.window.buffer_size[1]]
    ...
    def get_current_frame_as_image(self, alpha=False):
        ...
        width = self.window.fbo.viewport[2] - self.window.fbo.viewport[0]
        height = self.window.fbo.viewport[3] - self.window.fbo.viewport[1]
        image = Image.frombytes(
            fmt,
            (width, height),
            fbo.read(viewport=self.window.fbo.viewport, alignment=1, components=components),
        )
        if width != self.window.size[0] or height != self.window.size[1]:
            image = image.resize(self.window.size, Image.NEAREST)
    ...
    def get_current_depth_image(self, camera: CameraInterface):
        ...
        depth = Image.frombytes(
            "F",
            (width, height),
            fbo.read(viewport=self.window.fbo.viewport, alignment=1, attachment=-1, dtype="f4"),
        )

        if width != self.window.size[0] or height != self.window.size[1]:
            depth = depth.resize(self.window.size, Image.NEAREST)
    ...
    def get_current_mask_ids(self, id_map: Dict[int, int] = None):
        ...
        id = Image.frombytes(
            ...
            self.offscreen_p.read(
                viewport=self.window.fbo.viewport,
                alignment=1,
                components=1,
                attachment=1,
                dtype="f4",
        ...
        if width != self.window.size[0] or height != self.window.size[1]:
            id = id.resize(self.window.size, Image.NEAREST)
