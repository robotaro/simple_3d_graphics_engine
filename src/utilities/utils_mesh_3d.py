...
def from_obj(fpath: str, scale=1.0) -> tuple:
    ...
    #scene = pywavefront.Wavefront(fpath)

    vertices, normals, uvs, indices = load_obj(filename=fpath)

    """vertices, normals, uvs, indices = None, None, None, None

    try:
        vertices = np.array(mesh.vertices, dtype=np.float32)
    except AttributeError as error:
        vertices = np.array(mesh.triangles, dtype=np.float32)
