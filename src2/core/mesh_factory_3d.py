...
class MeshFactory3D:
    ...
    def generate_mesh(self, shape_list: list) -> DataGroup:
        ...
        new_data_group = DataGroup(archetype="mesh")
        new_data_group.data_blocks[KEY_PRIMITIVE_VERTICES] = DataBlock(data=np.concatenate(vertices_list, axis=0))
        new_data_group.data_blocks[KEY_PRIMITIVE_NORMALS] = DataBlock(data=np.concatenate(normals_list, axis=0))
        new_data_group.data_blocks[KEY_PRIMITIVE_COLORS] = DataBlock(data=np.concatenate(colors_list, axis=0))
        if len(indices_list) > 0:
            new_data_group.data_blocks[KEY_PRIMITIVE_INDICES] = DataBlock(data=np.concatenate(indices_list, axis=0))
